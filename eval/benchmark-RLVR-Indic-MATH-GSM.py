...
async def evaluate_model(model, dataset, semaphore):
    ...
    for i, data in enumerate(dataset):
        system_prompt = [
            {"role": "system", "content": "You are a helpful assistant. Think deeply before answering the user's question."}
        ]
        messages = system_prompt + data["translated_messages"]
    ...
    if len(verification_results) == 0:
        return 0, 0, invalid_results
    
    return sum(verification_results)/len(verification_results), len(verification_results), invalid_results
...
async def main(models, dataset):
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            continue
        print(f"Model: {models[i]['model']} | Accuracy: {result[0]} | Number of samples: {result[1]}") # noqa
        print(f"Invalid results: {result[2]}")
...
if __name__ == "__main__":
    ...
    models = [
        ...
        #     "model_url": "http://10.67.27.15:8085/v1/chat/completions",
        # },
        # {
        #     "model": "o3-mini",
        #     "model_url": "https://api.openai.com/v1/chat/completions",
        ...
        #     "model_url": "http://10.67.27.7:8084/v1/chat/completions",
        # },
    ]

    dataset = load_dataset("sarvam/RLVR-Indic-MATH-GSM").shuffle(seed=42)["test"]
    # models = [models[-1]]
